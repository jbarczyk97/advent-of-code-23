...
impl Hand {
    ...
    fn main_order(&self) -> usize {
        ...
        let joker_count_ref = counts.entry(1).or_insert(0);
        let joker_count = *joker_count_ref;
        let max_key = counts.iter().filter(|(&key, _)| key != 1).max_by_key(|&(_, val)| val).map(|(key, _)| *key).unwrap_or(0);

        let entry = counts.entry(max_key).or_insert(0);
        *entry += joker_count;
